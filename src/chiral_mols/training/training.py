...
def make_training_dir(run_name: str) -> Path:
    ...
    training_run_dir = Path("/share/snw30/projects/chiral_mols/training_runs")
...
def evaluate(
    ...
    for batch in dataloader:
        ...
        running_loss += (
            loss_fn(logits, batch.chirality_labels).item()
            * batch.chirality_labels.numel()
        )
        n_samples += batch.chirality_labels.numel()
    ...
    if return_full_output:
        eval_out = EvalOutput(
            chiral_embedding=torch.cat(all_embeddings),
            model_logits=torch.cat(all_logits),
            class_labels=torch.cat(all_labels),
            predictions=torch.cat(all_logits).argmax(dim=1).detach().cpu(),
        )
